...
// AccessLog 记录请求参数与响应预览（明文）。
// 无参即可用：r.Use(AccessLog())
func AccessLog(o ...AccessLogOptions) gin.HandlerFunc {
...
func (a *accessLog) handle(c *gin.Context) {
	if a.skipped(c) {
		c.Next()
		return
	}
	...
	n := contentLength(c.Request)
	line := fmt.Sprintf("\r\n--> %s %s", c.Request.Method, buildFullURL(c.Request))
	if n > 0 {
	...
	if params := a.formatParams(c); params != "" {
		prefix.WriteString("  [Params] " + params + "\n")
	}

	w := &captureWriter{body: bytes.NewBuffer(nil), ResponseWriter: c.Writer}
	c.Writer = w
	...
	body := decodeBody(w.body.Bytes())
	length := int64(w.body.Len())
	if body != "" {
	...
	failed := false
	if a.failureKey != "" {
		if raw := contextString(c, a.failureKey); raw != "" {
			failed = true
			a.writeFailure(&out, raw, contextError(c, a.errorKey))
		}
	}

	msg := out.String()
	if failed || status < 200 || status >= 300 {
		a.warn(c.Request.Context(), msg)
	} else {
		a.info(c.Request.Context(), msg)
	}
}

func (a *accessLog) skipped(c *gin.Context) bool {
	if c == nil || c.Request == nil || c.Request.URL == nil {
		return false
	}
	return util.MatchPath(c.Request.URL.Path, a.skipPaths)
}

func (a *accessLog) formatParams(c *gin.Context) string {
	contentType := strings.ToLower(GetHeader(c, request.HeaderContentType))
	if strings.Contains(contentType, "multipart/form-data") {
		if n := contentLength(c.Request); n > 0 {
	...
	if bodyBytes, exists := c.Get(gin.BodyBytesKey); exists {
		if body, ok := bodyBytes.([]byte); ok && len(body) > 0 {
			...
			return "body=" + a.truncate(string(body), a.maxParam)
		}
	}
...
func contextError(c *gin.Context, key string) string {
	if key == "" {
		return ""
	}
	if v, ok := c.Get(key); ok {
...
// captureWriter 包装 gin.ResponseWriter，同时记下响应体。
